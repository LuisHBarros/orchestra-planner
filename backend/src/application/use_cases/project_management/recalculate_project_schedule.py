...
class RecalculateProjectScheduleUseCase:
    ...
    async def execute(self, input: RecalculateProjectScheduleInput) -> ProjectSchedule:
        ...
        async with self.uow:
            ...
            deps = await self.uow.task_dependency_repository.find_by_project(
                input.project_id
            )
            members = await self.uow.project_member_repository.find_by_project(
                input.project_id
            )
            ...
            if tasks_to_save:
                await self.uow.task_repository.save_many(tasks_to_save)
