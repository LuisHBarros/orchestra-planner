...
from backend.src.domain.entities import SeniorityLevel, Task
...
class RecalculateProjectScheduleUseCase:
    ...
    async def execute(
        self, input: RecalculateProjectScheduleInput
    ) -> ProjectSchedule:
        """
        ...
        deps = await self.task_dependency_repository.find_by_project(
            input.project_id
        )
        members = await self.project_member_repository.find_by_project(
            input.project_id
        )
