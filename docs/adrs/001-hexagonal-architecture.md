...
## Decision
Adopt the **Hexagonal Architecture** (Ports and Adapters) to separate the domain from external layers.
...
## Consequences
- The domain layer remains dependency-free.
- Adapters implement all external interfaces (DB, HTTP, LLM, notifications).
- Use cases and ports are the primary integration points for infrastructure.
